...
test('导演只接受批准模型且响应必须报告同一模型', () => {
  const run = (model) => (args) => {
    assert.equal(args[args.indexOf('--model') + 1], DIRECTOR_MODEL);
    return { status: 0, stdout: JSON.stringify({ model, choices: [{ message: { content: '{"version":6,"units":[]}' } }] }) };
  };
  assert.equal(callDirector('brief', { run: run(DIRECTOR_MODEL) }).responseModel, DIRECTOR_MODEL);
  assert.equal(callDirector('brief', { run: run(undefined) }).ok, false);
  assert.equal(callDirector('brief', { run: run('local') }).ok, false);
  assert.throws(() => callDirector('brief', { model: 'qwen3.5:27b', run: run(DIRECTOR_MODEL) }), /高级模型/);
});
