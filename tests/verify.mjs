 ...
 *
 * 对每个包依次：
 *   1. 用 profile 目录做解析基准，证明包能被 DSH 解析到（和 dsh-memory 同一条路）；
 ...
 *
 * 用法：node plugin/verify.mjs [包名 ...]     （默认两个包都验）
 */
...
/** 置 1 时从工作区源码目录解析，用来在装进 profile 之前先验证。 */
const FROM_WORKSPACE = process.env.VERIFY_FROM_WORKSPACE === '1';
const WORKSPACE_PLUGIN_DIR = 'E:\\AI-Tool\\DeepSeek\\story2video\\plugin';
...
const EXPECT = {
  'dsh-storyboard': {
    ...
    panelProbe: {
      expectPath: 'story2video/examples/demo.storyboard.json',
      board: {
        meta: {
        ...
        characters: [{ id: 'boy', name: '小宇', appearance: '东亚男孩，圆脸', wardrobe: '红短袖', ref_image: 'story2video/shots/probe_char.png' }],
        scenes: [{ id: 'field', name: '球场', environment: '黄昏草地', ref_image: 'story2video/shots/probe_scene.png' }],
        props: [{ id: 'ball', name: '旧足球', description: '白色带黑五边形的旧足球', ref_image: 'story2video/shots/probe_ball.png' }],
        shots: [
  ...
  'dsh-media-preview': {
    ...
    probe: {
      address: 'dsh-resource://file/absolute/E:/AI-Tool/DeepSeek/comfy-out/i2v_20260912-204558.mp4',
      expectTag: 'video',
...
const targets = process.argv.slice(2).length ? process.argv.slice(2) : Object.keys(EXPECT);
...
for (const name of targets) {
      ...
      workspaceFiles: {
        ...
        list(sessionId, dirPath, signal) {
          ...
          return Promise.resolve({ ok: true, value: { entries: [
            { name: 'demo.storyboard.json', type: 'file' },
            { name: 'other.storyboard.json', type: 'file' },
  ...
  for (const { registration, Component } of registered.slots) {
    ...
    if (expect.panelProbe) {
      ...
      check('请求路径正确', remoteCalls[0] && remoteCalls[0].path === probe.expectPath, String(remoteCalls[0] && remoteCalls[0].path));
      ...
      check('选项来自目录扫描（2 个 .storyboard.json）', options.length === 2, `实际 ${options.length}`);
      ...
      const other = 'story2video/examples/other.storyboard.json';
