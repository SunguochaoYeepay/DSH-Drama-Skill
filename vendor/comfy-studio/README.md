...
**`gen.py` 的 `COMFYUI_ROOT` 默认值**（行号见文件内注释）：上游写的是某台机器的 ComfyUI
安装根目录，本仓库改成空串并加注释。

- **为什么改**：写死的本机路径对 clone 者毫无意义（那块盘不存在），还会把原来的机器信息带进公开仓库。
- **会不会影响行为**：不会。这个值只用来定位 ComfyUI 自己的产物目录，缺省时退化成当前目录；
  真正要用就设 `COMFYUI_ROOT` 环境变量，或由调用方显式传 `--comfy-root`。
- **同步上游时注意**：重拷会把这条改动冲掉，记得补回来（它是唯一一处偏离上游的编辑）。
