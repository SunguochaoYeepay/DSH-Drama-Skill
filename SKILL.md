...
2. 剧本生成后，展示完整剧本和结构检查结果，**并附一段白话预演**。预演要同时回答**故事三问**：
   ...
   白话预演**按剧本的场面/段落讲，不是导演稿的 shot 级** —— 这时导演稿还不存在。每一段说明画面上会看到什么、观众此刻知道什么、角色此刻知道什么。**只给文本是不够的**：因果缺口和物理错误在文本上看不出来（文字甚至可能是对的），只有把"这一段会怎么拍"讲成白话才暴露。**三问的答案就是这段预演本身** —— **"剧本里没写"不是默认，是缺口**（`pot_hit` 2026-09-20 实测：这三问都没问，代价是 44 分钟返工加一次重做）。见 [`references/workflow.md`](references/workflow.md) 的故事闸门。
