...
每部剧一律建在**本仓库内**的 `projects\<剧名>\` 目录下；禁止在其他磁盘或临时目录自建项目根。

仓库根就是**唯一的工作根**（2026-09-22 定型）。历史上跑在仓库外的 `story2video` 副本（例如别人的另一块盘上的 `<…>\projects\`）已整体迁入，别再往那里写 —— clone 本仓库的人手上没有那块盘。

新建立项前先列一次该 `projects\` 目录：已有同名副目录就续用并先读其状态，没有才新建。
...
- 剧目的产物只在本机 `projects\` 下（该目录整体不入 git）；确定性测试只使用仓库内 `tests/fixtures/` 的最小夹具，不依赖某个用户项目。
