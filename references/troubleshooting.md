...
- 保存原片，再生成去字幕版本。
- 两条通道并存，按机器条件选：`cli/desub.mjs`（VSR，Docker、句子长但快）与 `cli/desub-void.mjs`（VOID，本机 ComfyUI、免 Docker）。
- 两条都**必须显式给字幕带**（`--top/--bottom`，VOID 再加 `--left/--right`）。默认值只是兜底，用默认值等于赌字幕恰好在那个位置。
- VOID 通道可以 `--auto-band` 自动找带（`tools/band_detect.py`：靠"亮+细+带描边"的字形特征，不认字）；显式参数覆盖它，找不到候选时它拒绝跑而不是蒙一个位置。
- VOID 通道跑完会自带核查：**带内差 ≫ 带外差**才算成了（阈值与底噪口径在 `src/video-diff.mjs`）。这条核查能挡住两类不报错的翻车 —— 字幕根本没动、掩码极性反了把整帧重画。单独核查任意两个视频用 `cli/band-diff.mjs`。
