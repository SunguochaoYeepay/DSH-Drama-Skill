...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 兜底的分镜文件，相对会话工作区根。 */
		const BOARD_PATH = "story2video/examples/demo.storyboard.json";
		/** 找分镜文件时依次尝试的目录。 */
		const BOARD_DIRS = ["story2video/examples", "story2video", "."];
		/**
		 * 流水线写的「当前板子」指针。**面板先读它，读不到才扫目录。**
		 * 必须有它：`workspaceFiles.list` 不返回修改时间，扫目录没法判哪个是在用的。
		 */
		const POINTER_PATH = "story2video/current.json";
		 ...
		 * 依次扫候选目录，收集其中的分镜文件。
		 *
		 * **先读「当前板子」指针** `story2video/current.json` —— 它是流水线写的，
		 * 明确指向正在制作的那一块板子。
		 *
