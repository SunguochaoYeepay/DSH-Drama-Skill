...
const STYLE_ANCHOR = {
  ...
  //
  // 🔁 **这条原先是唯一带否定词串的锚，实测把画面推成了它的反面**（2026-09-17 after_waking）：
  //    原串结尾是「不要写实照片感、不要真人皮肤纹理、不要 2D 平涂与赛璐璐描线」，
  //    本地通道出来的肖像与场景主图**全是写实摄影**，而两张正是全片的风格锚。
  //    这与 `references/preflight.md` 第 4.3 条「扩散模型对否定句的处理常等同于正向提示」
  //    以及 `references/video-h3.md` 的「妆面污染」同一机制。
  //
  // 🔁 **光删否定词还不够 —— 正向描述必须写到"具体形状"这一层**（同日，同一剧第二轮）：
  //    删掉否定词的版本（「3D 卡通动画电影长片质感，三维渲染管线，圆润饱满的造型…」）
  //    在**两个通道上都仍然出写实**：
  //      · 本地：走 `--fast`（cfg 1.0）时负向条件失效，画面被 Lightning LoRA 拉回写实；
  //        只有 `--steps 20` + `--style anime` 才出卡通。
  //      · 百炼：没有预设可用，纯靠这段锚 —— 上面那版**依然出写实 CG**。
  //    把锚改成下面这版（点名卡通造型的**具体特征**：大眼睛、圆润面部轮廓、
  //    简化夸张的五官比例、平滑卡通皮肤、饱满体型）后，**百炼一次到位**出 3D 卡通。
  //    所以规则是：**「3D 卡通」是个风格名，不是画面描述** —— 模型不认名词，只认形状。
  cartoon3d: '欧美三维卡通动画长片风格，皮克斯迪士尼式卡通造型：大眼睛、圆润饱满的面部轮廓、'
    + '简化并适度夸张的五官比例、平滑的卡通皮肤、饱满的卡通体型比例，三维渲染，'
    + '柔和体积光与柔阴影，高饱和动画配色，玩具般的材质质感',
...
export function sheetInstruction(board, identity, contract = null) {
  ...
  return glue(head, [
    ...
    `【站位】每一格里的${human ? '人物' : '角色'}都**水平居中、脚踩同一条水平线、头顶留同样的空隙**，四格${human ? '人物' : '角色'}高度一致。`,
    ...
    human
      ...
      : 'Panel1 必须是 Panel2 头部的放大，五官、毛色与斑纹完全对得上。',
    // 实测教训：肖像参考图身上那件"素色上衣"会被一并继承过去，
    // 导致 4 个面板里只有脸是对的、身上还是 T 恤。参考图只该用来锁脸。
    //
    // 🔁 **发型也必须从参考图里摘出去**（2026-09-17 after_waking，同一类问题的第二次）：
    //    原句写的是「锁定长相（脸型、五官、**发型**）」—— 但按板子契约，
    //    **发型属于 `identities[].appearance_details`，不属于 `characters[].portrait`**。
    //    把发型算成参考图属性 = 假设一个角色只有一种发型。
    //    实测换发型/换性别的剧里：肖像（短寸头男）把「梦中女装」的身份图 Panel1
    //    锁成了短发男脸，而 Panel2-4 按服装描述画成卡通红裙长发女孩 ——
    //    **同一张身份图内部两个身份**，关键帧再照着 Panel1 锁脸，就把男孩一路带到成片。
    human
