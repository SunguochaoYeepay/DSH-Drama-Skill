 ...
 *
 * **2026-09-24 再放宽到 800（用户决定）**：理由是**过肩/遮挡这类空间结构句**要写足
 * 「前景是谁的哪个部位的背影、占画面哪一侧多少、主体在对面朝哪、桌面与道具落在哪」，
 * 500 装不下（table_for_two 的 g001 用 619 字的过肩写法出图正确；同镜改用 500 字以内的
 * **抽象关系句**「两人隔着餐桌面对面坐」反而画成了并排同侧 ✗）。
 *
 * ⚠ **放宽上限不等于鼓励写长**：实测把"最高优先级/绝对不允许"这类**强调句**加进去，
 * 出片率从 83% 掉到 67%。预算该花在**能画成像素的几何与材质**上，而不是强调词上。
 */
